        ...
        if is_exact_fpl_bootstrap(source):
            ...
            for element in elements if isinstance(elements, list) else []:
                ...
                if isinstance(news, str) and news.strip():
                    excerpts.append({"kind": "json_field", "text": news[:4096], "captured_at_utc": captured_at})
...
def fetch_source(source, opener=None, sleeper=None, now=None):
    ...
    for attempt in range(2):
        try:
            ...
            title, excerpts = extract_metadata(body, content_type, captured_at, source)
            return "captured", {"title": title, "excerpts": excerpts, "retrieved_at_utc": captured_at}, None
...
def collect(config=None, packet=None, opener=None, sleeper=None, now=None):
    ...
    for configured in source_config(config):
        ...
        if reason:
            ...
            state, metadata, code = fetch_source(configured, opener, sleeper, now)
            if state == "captured":
...
def run(config=None, path="data/research_packet.json", opener=None, sleeper=None, now=None):
    config = config or load()
    previous = load_packet(path)
    try:
        packet = collect(config, previous, opener, sleeper, now)
    except ValueError:
